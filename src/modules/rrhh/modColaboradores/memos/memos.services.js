...
export const uploadMemoFileToLaravel = async (file) => {
  const formData = new FormData();
  formData.append('file', file.buffer, file.originalname);

  const uploadUrl = `${process.env.LARAVEL_URL}/api/memos`;
 ...
 */
export const deleteMemoFileFromLaravel = async (filename) => {
  if (!filename) return;

  const deleteUrl = `${process.env.LARAVEL_URL}/api/memos`;
